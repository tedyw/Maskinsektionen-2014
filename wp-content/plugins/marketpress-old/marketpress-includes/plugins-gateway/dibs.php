<?php
/*
MarketPress Dibs Plugin
Author: Tedy Warsitha (Code Origin)
*/

class MP_Gateway_Dibs extends MP_Gateway_API {

  //private gateway slug. Lowercase alpha (a-z) and dashes (-) only please!
  var $plugin_name = 'dibs';
  ...
  //credit card vars
  var $API_Username, $API_Password, $API_Merchant, $SandboxFlag, $returnURL, $cancelURL, $API_Endpoint, $version, $currencyCode, $locale;
  ...
  function on_creation() {
    ...
    require_once($mp->plugin_dir .'plugins-gateway/dibs-files/DIBSFunctions.php');

    $settings = get_option('mp_settings');

    //set names here to be able to translate
    $this->admin_name = __('Dibs', 'mp');
    ...
    if ( isset( $settings['gateways']['dibs'] ) ) {
      $this->API_Merchant = $settings['gateways']['dibs']['api_merchant'];
      $this->API_Username = $settings['gateways']['dibs']['api_user'];
      $this->API_Password = $settings['gateways']['dibs']['api_pass'];
      //$this->API_Signature = $settings['gateways']['dibs']['api_sig'];
      $this->currencyCode = $settings['gateways']['dibs']['currency'];
      //$this->locale = $settings['gateways']['dibs']['locale'];
      //set api urls
      //if ($settings['gateways']['dibs']['mode'] == 'sandbox')	{
      //  $this->API_Endpoint = "https://pilot-dibspro.paypal.com";
      //} else {
      //  $this->API_Endpoint = "https://dibspro.paypal.com";
      //}
    }
  }
  ...
  function process_payment($cart, $shipping_info) {
    ...
    $RES = DIBSAuth($settings['gateways']['dibs']['api_merchant'],$total,$this->currencyCode,$_SESSION['card_num'],$_SESSION['exp_month'],substr($_SESSION['exp_year'],2,2),$_SESSION['card_code'],$_SESSION['mp_order'],"ssl",$settings['gateways']['dibs']['api_md5'],$settings['gateways']['dibs']['testing']);
    ...
    if ( $RES['authinfo']['transact'] != "" ) {
      ...
      $payment_info['currency'] = $this->currencyCode; // Authorize.net only supports USD transactions
      $payment_info['transaction_id'] = $RES['transact'];

      //succesful payment, create our order now
      $result = $mp->create_order($_SESSION['mp_order'], $cart, $shipping_info, $payment_info, $paid);
    } else {
      $mp->cart_checkout_error( var_dump($RES) );
    }
  }
  ...
  function gateway_settings_box($settings) {
    ...
    if ( !isset( $settings['gateways']['dibs'] ) )
    	$settings['gateways']['dibs'] = array();

    $settings['gateways']['dibs'] = $settings['gateways']['dibs'] + $defaults;

    ?>
    <div id="mp_dibs" class="postbox">
      <h3 class='hndle'><span><?php _e('Dibs Settings', 'mp'); ?></span></h3>
      <div class="inside">
				<a href="http://tech.dibspayment.com/" target="_blank"><img src="<?php echo $mp->plugin_url . 'images/dibs.jpg'; ?>" /></a>
        <p class="description"><?php _e('Nordic market leading solution for secure Online Payments.', 'mp') ?></p>
        <table class="form-table">
				  <tr>
				    <th scope="row"><?php _e('Testing', 'mp') ?></th>
				    <td>
			        <p>
			          <select name="mp[gateways][dibs][testing]">
			            <option value="yes" <?php selected($settings['gateways']['dibs']['testing'], 'yes') ?>><?php _e('Yes', 'mp') ?></option>
			            <option value="no" <?php selected($settings['gateways']['dibs']['testing'], 'no') ?>><?php _e('No', 'mp') ?></option>
			          </select>
			        </p>
				    </td>
				  </tr>
				  <tr>
					...
					<label><?php _e('USER', 'mp') ?><br />
					  <input value="<?php echo esc_attr($settings['gateways']['dibs']['api_user']); ?>" size="30" name="mp[gateways][dibs][api_user]" type="text" />
					</label>
				      </p>
				      <p>
					<label><?php _e('PWD', 'mp') ?><br />
					  <input value="<?php echo esc_attr($settings['gateways']['dibs']['api_pwd']); ?>" size="30" name="mp[gateways][dibs][api_pwd]" type="password" />
					</label>
				      </p>
              <p>
          <label><?php _e('MERCHANT', 'mp') ?><br />
            <input value="<?php echo esc_attr($settings['gateways']['dibs']['api_merchant']); ?>" size="30" name="mp[gateways][dibs][api_merchant]" type="text" />
          </label>
              </p>
              <p>
          <label><?php _e('MD5 K1', 'mp') ?><br />
            <input value="<?php echo esc_attr($settings['gateways']['dibs']['MD5']['K1']); ?>" size="30" name="mp[gateways][dibs][MD5][K1]" type="text" />
          </label>
              </p>
              <p>
          <label><?php _e('MD5 K2', 'mp') ?><br />
            <input value="<?php echo esc_attr($settings['gateways']['dibs']['MD5']['K2']); ?>" size="30" name="mp[gateways][dibs][MD5][K2]" type="text" />
          </label>
              </p>
          ...
          <tr>
          <th scope="row"><?php _e('Dibs Currency', 'mp') ?></th>
          <td>
            <select name="mp[gateways][dibs][currency]">
            <?php
            $sel_currency = ($settings['gateways']['dibs']['currency']) ? $settings['gateways']['dibs']['currency'] : $settings['currency'];
            $currencies = array(
            ...
            ?>
					...
					<label><?php _e('Email Customer (on success):', 'mp'); ?><br />
			                  <select name="mp[gateways][dibs][email_customer]">
			                    <option value="yes" <?php selected($settings['gateways']['dibs']['email_customer'], 'yes') ?>><?php _e('Yes', 'mp') ?></option>
			                    <option value="no" <?php selected($settings['gateways']['dibs']['email_customer'], 'no') ?>><?php _e('No', 'mp') ?></option>
			                  </select>
			                </label>
			              </p>
					...
					<label><a title="<?php _e('This text will appear as the header of the email receipt sent to the customer.', 'mp'); ?>"><?php _e('Customer Receipt Email Header', 'mp'); ?></a><br/>
			                  <input value="<?php echo empty($settings['gateways']['dibs']['header_email_receipt'])?__('Thanks for your payment!', 'mp'):esc_attr($settings['gateways']['dibs']['header_email_receipt']); ?>" size="40" name="mp[gateways][dibs][header_email_receipt]" type="text" />
			                </label>
				      </p>
					...
					<label><a title="<?php _e('This text will appear as the footer on the email receipt sent to the customer.', 'mp'); ?>"><?php _e('Customer Receipt Email Footer', 'mp'); ?></a><br/>
			                  <input value="<?php echo empty($settings['gateways']['dibs']['footer_email_receipt']) ? '' : esc_attr($settings['gateways']['dibs']['footer_email_receipt']); ?>" size="40" name="mp[gateways][dibs][footer_email_receipt]" type="text" />
			                </label>
				      </p>
...
mp_register_gateway_plugin( 'MP_Gateway_Dibs', 'dibs', __('Dibs', 'mp') );
?>